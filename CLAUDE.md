...
**Current stage:** Visual overhaul V-9 complete (La Cucina kitchen overhaul). Music system migrated to file-based. Intro animation added. See `HANDOFF.md`.
...
| 23 | 2026-03-27 | V-5 | PixelLab asset generation (Phases 4-7): (1) Phase 4 — 18 NPC chibi sprites generated and downloaded (south-facing PNGs). Manifest NPC keys remapped to match world.js npc.id values (e.g. chef_tutorial, librarian, gym_trainer). 3 NPCs without sprites use procedural fallback (market_vendor, market_cat_lady, canal_fisherman). (2) Phase 5 — 4 boss/enemy sprites: boss-enzo.png, boss-bridget.png, enemy-goon.png, broom.png. (3) Phase 6 — 16 item sprites as individual PNGs: 8 weapons (flour, tomato, banana, spatula, sock, rubber_duck, rolling_pin, cdrom), 7 power-ups (broccoli, choco_milk, water, deli_meat, gouda, brownie, milk), 1 recipe fragment (shared by all 5). New `itemSprites` manifest section + `drawItemById()` in SpriteLoader. All 5 render paths updated (world items, HUD inventory, weapon HUD, power-up pickups, buff HUD). (4) Phase 7 — 3 UI sprites: heart_full.png, heart_empty.png, coin.png. drawUI() enhanced with scaling params. Hearts + coin HUD use PixelLab sprites with procedural fallback. Total: 41 new PNG assets. Cache-busting ?v=48. |

| 24 | 2026-03-28 | V-6 | Rendering polish (visual-overhaul-instructions.md): (1) Y-sorted render pipeline — all entities (NPCs, pushables, objects, enemies, bosses, broom, Brodo, player, items, power-ups) collected into flat array sorted by bottom-edge Y, rendered in depth order. Extracted renderSingle* functions for NPCs, enemies, pushables, objects, world items, power-ups. NPC/object interaction prompts split into renderNPCPrompt/renderInteractionPrompts. Ground effects (sparkles, traps) render before Y-sort; overlays (projectiles, boss attacks, weapon swing) render after. (2) Drop shadows on all objects — pushables (radiusX=13), objects (radiusX=10), world items (radiusX=8) via drawCharacterShadow(). (3) Wall top-face highlights — 2px bright strip at bottom of wall tiles facing open floor. (4) Ambient light gradient — subtle warm rgba(255,255,200) top-to-bottom overlay. (5) Multi-tile objects — connected counter variants (left/mid/right with stone top), 2x2 stove composite (burners + oven door), 2x2 stall overlay (candy-striped awning + goods), 1x2 tall bookshelf (4 rows of colored spines), tree decorations (2x3 canopy+trunk) in Market and Piazza via ZONE_DECORATIONS. Adjacency detection in renderTiles() overlay pass. (6) Tile transitions — gradient blend strips (6px wide, alpha 0.25) where different walkable tiles meet, using neighbor tile color. (7) BMX mini-game polish — 3rd far parallax hill layer, wispy high-altitude clouds, compound sine curves for organic hill profiles, scrolling dashed road center line, rotating wheel spokes. Cache-busting ?v=51. |

| 25 | 2026-03-28 | — | Intro animation: 8-slide cinematic story intro on New Game (restaurant, sisters, dogs, wedding, problem, fragments, Papa, go). Typewriter text reveal, fade transitions, scene-specific procedural illustrations, PixelLab sprite integration, slide progress dots, Z/Space advance, Escape skip. game.mode='intro'. Intro music hook (playSpecialMusic/stopSpecialMusic). |
| 25 | 2026-03-28 | — | Enemy/boss sprite regeneration: goon, boss-enzo, boss-bridget regenerated via PixelLab with heroic proportions (not chibi). drawEnemy/drawBoss now support displaySize parameter (goons 40px, bosses 48px). Broom regenerated as enchanted magical broom map object. |
| 25 | 2026-03-28 | — | HUD overhaul: debug info removed (FPS_DISPLAY=false). Inventory moved to bottom-right at 80% scale. Weapon/score/buff HUD moved to top-left. Item pickup flash changed from full-screen yellow wash to subtle golden vignette with dark-backed text. |
| 25 | 2026-03-28 | — | Music system: Tone.js procedural music replaced with Howler.js file-based playback from generated-music/ folder. 8 zone tracks, 2 boss fight tracks, 4 special tracks (intro, finale, credits, transition). startBossTempo/endBossTempo now swap to boss MP3 tracks. Tone.js fallback preserved if files fail to load. All SFX (footsteps, blips, weapons) remain Tone.js. Music ducking + volume control updated for both systems. |
| 25 | 2026-03-28 | — | Overlay tile system: barrel, flower, fountain tiles made transparent (procedural backgrounds removed). Tile renderer draws ground tile first for overlay types. TILES definitions updated with overlay+ground properties. |
| 25 | 2026-03-28 | V-7 | Visual overhaul pass 2 (VISUAL_OVERHAUL_BRIEF-2.md): (1) Flower — 5 color variants (pink/white/yellow/purple/salmon), shrunk to ~8px bloom, 1px stem, small leaf. (2) Grass — 8 variants with varied base colors, diagonal grass blade strokes, occasional flower pixel. (3) Walls — darkened ~20%, low-contrast mortar. (4) Floors — 4 warm wood plank variants with horizontal grain. (5) Gym tiles — MAT warm beige with seam lines + red cross, EQUIPMENT dark rubberized with barbell silhouette, MIRROR silver chrome frame with reflective gradient, JUICEBAR warm wood with drink cups + menu board. (6) Market stalls — 3 variants with red/blue/green striped awnings + produce dots. (7) Water — darker deep base, shimmer streaks, dark south depth edge. (8) Crate — 3D faces (lighter top, main front, dark bottom edge, wood grain). (9) Bridge planks — horizontal plank stripes with grain, darker edges, nail dots. (10) Dock — grey cobblestone with mortar grid, mossy patches, wet edge. (11) Large double-door overlay — detects adjacent DOOR tiles and renders connected arch door sprite. (12) Manifest cleanup — removed 404-causing entries for non-existent spritesheets. |
| 25 | 2026-03-28 | V-8 | PixelLab tileset regeneration: 32 tiles regenerated across 5 batches (8+6+8+8+4). Outdoor: grass, path, cobble, water, dock, plank, bridgegap, flower. Indoor: floor, wall, door, rug, shelf, stall. Gym: mat, equipment, juicebar, mirror. Kitchen: stove, oven, counter. Sewing: fabric, sewmach, mannequin, carpet, saucemach. Special: fountain, checkered, dining, barrel, filltarget. Assembled into new universal.png (160×224, 5×7 grid). Aggressive border removal (5px edge, sample from pixel 6) to eliminate dark tile seams. |
| 25 | 2026-03-28 | V-9 | La Cucina kitchen overhaul: 8 kitchen-specific tiles regenerated via PixelLab — warm cream plaster wall, terracotta floor, cute retro stove with red knobs, honey wood counter with herbs, bright shelf with colorful dishes, terracotta rug, honey wood arch door, checkered curtain window. 4 kitchen decoration objects generated — hanging copper pots, fruit bowl, pasta jar, kitchen sink. Decorations added to La Cucina zone (pots flanking stove, fruit on counter, pasta jars on shelves, sink). Cache-busting ?v=61. |
